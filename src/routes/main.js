...
router.get("/libros", async (req, res) => {
    ...
    res.render(`${config.views}/public/libros.ejs`, {products: products});
});
...
router.get("/news", async (req, res) => {
    ...
    try {
        products = await Products.getPaginatedBestSellers(page = 0);
    } catch (e) {
        throw e;
    }

    res.render(`${config.views}/public/news.ejs`, {products: products});
});
// Product order page
...
router.get("/booking", authenticate(), async (req, res) => {
    ...
    try {
        bookingContent = await booking.getContent(req.session.passport.user);
        bookingContent = JSON.parse(bookingContent.content);
        let idList = bookingContent.map(({ id }) => id)
        idList = Array.from(new Set(idList)).toString();
        products = await Products.getByIdArray(idList);
    } catch {
    ...
    if (bookingContent) products = JSON.parse(JSON.stringify(products))
    res.render(`${config.views}/public/booking.ejs`, {booking: bookingContent, products: products});
});
