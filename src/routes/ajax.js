...
router.get("/loadPageNews", async (req, res) => {
    const ProductsController = require('../controllers/products.js');
    const Products = new ProductsController();
    let products;

    try {
        products = await Products.getPaginatedNews(req.query.page);
    } catch (e) {
        products = false;
    }

    res.render(`${config.views}/templates/bestsellers.ejs`, {products: products});
});
...
router.post("/updatebooking", async (req, res) => {
    ...
    let response = await booking.update(req.body.updateProduct, req.session.passport.user);
    res.send(response)
});
...
router.post("/addTobooking", async (req, res) => {
    ...
    try {
        response = await booking.addTobooking(req.body.addTobooking, req.session.passport.user);
    } catch (e) {
        response = e;
    }
