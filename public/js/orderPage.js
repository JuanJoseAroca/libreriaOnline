...
document.querySelector('a[href="/libros"]').classList.add('active');
...
document.querySelector('#cartAdd').addEventListener('click', () => {
    ...
    if (addToCart.length > 0) {
        ...
        fetch(stockCheck)
            .then(() => {
                options.title = 'Super! 📚';
                options.content = 'Productos agregados al carrito!';
        ...
        options.title = 'Oops... ❌';
        options.content = 'No hay productos seleccionados';
        $('#cartAdd').popover('dispose').popover(options).popover('show');
        setTimeout(() => $('#cartAdd').popover('hide'),2200);
    }
});

document.querySelector('#bookingAdd').addEventListener('click', () => {
    const id =  document.querySelector('div[data-product]').dataset.product;
    const sizes = document.querySelectorAll('div[data-size]');
    const addTobooking = [];
    for (let size of sizes) {
        let quantity = parseInt(document.querySelector('#counter-'+size.dataset.size).textContent);
        if (quantity > 0 ) {
            addTobooking.push({'id': id, 'size': size.dataset.size, 'quantity': quantity});
        }
    }
    let options = {
        trigger: 'manual',
    ...
    if (addTobooking.length > 0) {
        var stockCheck = new Request('/ajax/addTobooking', {
            headers: { "Content-Type": "application/json" },
            method: 'POST',
            body: JSON.stringify({'addTobooking':addTobooking})
        });

        fetch(stockCheck)
            .then(() => {
                options.title = 'Bien! ⏰ ';
                options.content = 'Productos Reservados!';
                $('#bookingAdd').popover('dispose').popover(options).popover('show');
                setTimeout(() => $('#bookingAdd').popover('hide'),2200);
                $('span[id^="counter"]').text('0');
            });
    } else {
        options.title = 'Oops... ❌';
